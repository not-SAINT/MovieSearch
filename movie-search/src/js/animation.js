export const showSlides = () => {
  setTimeout(() => {
    document.querySelectorAll('.movie-card').forEach((card) => card.classList.add('movie-card__show'));
  }, 0);
};

export const hideSlides = () => {
  document.querySelectorAll('.movie-card').forEach((card) => card.classList.remove('movie-card__show'));
};
